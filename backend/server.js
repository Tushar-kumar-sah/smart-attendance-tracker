...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    res.json({ token, role: user.role, userId: user.userId, name: user.name });
  } catch (err) {
    console.error(err);
...
app.post('/api/routine', async (req, res) => {
  try {
    ...
    for (const [day, periods] of Object.entries(req.body.routine)) {
      await Routine.create({ day, periods, periodsCount: req.body.periodsCount, userId: req.body.userId });
    }
...
app.get('/api/attendance', async (req, res) => {
  try {
    ...
    const map = {};
    records.forEach(r => { map[r.subjectName] = r.status; });
    res.json(map);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/admin/subjects', auth, isAdmin, async (req, res) => {
  try {
    ...
    const sub = new Subject({ subjectName: name, teacherName: teacher, userId: 'admin' });
    await sub.save();
    res.json(sub);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/api/admin/attendance-stats', auth, isAdmin, async (req, res) => {
  try {
    ...
    res.json({ totalStudents, attendanceRecords: stats });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
